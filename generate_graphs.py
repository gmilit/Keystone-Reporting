...
def main(show: bool = False):
    ...
    # 1️⃣ Support tickets (resolved / unresolved)
    ...
    # 2️⃣ Red incidents
    p1p2_issues = jira.search_issues(
        f'{jql_window} AND summary ~ "REC ISSUE"',
        maxResults=False,
        fields="created",
    )
    p1p2_series = weekly_series(p1p2_issues)
    save_fig(p1p2_series, "Weekly Rec Incidents", "p1p2.jpg", colors=JIRA_GREEN)
